...
void ratiosGen(TString Dss="", int expo=0, int expTex=2){
  ...
  int lund[] = {421,423};
  TString texName = "babar_code/Tables/texRatioGen"; texName+=Dss;texName+="DEff.txt";
  fstream tex;
  tex.open(texName,fstream::out);
  TString txName = "babar_code/Tables/tx";txName+=Dss;txName+="DEff.txt";
  fstream txCoc;
  txCoc.open(txName,fstream::in);
  TString txGenName = "babar_code/Tables/tx";txGenName+=Dss;txGenName+="DEffGen.txt";
  fstream txGen;
