...
void Table_David(TString folder, TString otherTag = "NN"){
  ...
  fstream Table;
  Table.open("mycode/tables/tableD_"+folder+".txt",fstream::out);
  fstream tex;
  tex.open("mycode/tables/textableD_"+folder+".txt",fstream::out);

  otherTag = "M14";

  int DYields[] = {133,8,2848,99,104,69,0};
  int MYields[] = {131,16,2798,72,97,79};
  int M22Yields[] = {171,7,3201,91,111,118};
  ...
  int M22Yields2[] = {111,140,2891,1198,294,258};
  ...
  int M22Yields3[] = {73,2,1654,7,72,64};
  ...
  int M22Yields4[] = {51,36,533,518,95,93};
...
void toTable(int cand, TChain &ch, int *DYields, int *MYields, int *M22Yields, TString *TaNames, 
  ...
  Table<<" "<<TaNames[7]<<"\t  "<<folder<<"\t David\t  M/D\t "<<otherTag<<endl;
  Table<<"==================================================="<<endl;
  tex<<"\\begin{tabular}{|l||cc|c|}"<<endl<<"\\hline"<<endl;
  tex<<"$\\mathbf{"<<TeNames[7]<<"}$ & M22 & D22 & M22/D22 \\\\"<<endl;
  ...
  for(int i=0; i<6; i++){
    ...
    Table<<TaNames[i]<<"\t  "<<entries<<"\t "<<DYields[i]<<"\t  "<<round((double)entries,(double)DYields[i])<<endl;
    tex<<TeNames[i]<<entries<<" & "<<DYields[i]<<" & "<<round((double)(entries),(double)DYields[i])<<" \\\\"<<endl;
  }
  Table<<endl;
