...
//      $Id: prdFit.cc,v 1.1 2012/08/23 02:22:18 manuelf Exp $
//
// Description:
...
void PlotFinalFit2(TString textName, TTree *tree, int nbins, double minx, double maxx, double minY, double maxY,
  ...
  style.xTitleOffset = 0.97; style.yTitleOffset = 1.;
  style.PadBottomMargin = 0.21; style.PadLeftMargin = 0.2;
  style.TextSize = 0.1; style.TitleSize = 0.1;  style.LabelSize = 0.095; 
  style.applyStyle();
  ...
  for(int pad=begChan; pad<endChan; pad++){
	...
	if(isDss && !typePlot.Contains("histo")){
	  ...
	  style.setTitles(htree[pad+4],xTitle2,ytitle2,PadLabel[IndPad[doPl][isIso][pad]]);
      } else style.setTitles(htree[pad+4],xTitle2,ytitle2,channelTitle[pad],rightLabel);
    if(typePlot.Contains("Leg") && (IndPad[doPl][isIso][pad]==begChan+3 || typePlot.Contains("Sum"))) {
      leg[pad]->Draw();
      didLeg = 1;
    }      
    ...
    if(style.isThesis=="yes") style.setTitles(htree[pad],xTitle,ytitle,"",PadLabel[IndPad[doPl][isIso][pad]-1]);
  ...
  if(Subtract.Contains(".ps")) {
    ...
    TString plotName = "keys/eps/FinalFit/PRD_"; plotName+=sample; plotName+=var; plotName+="_"; 
