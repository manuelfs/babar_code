...
//      $Id: KeysUtils.cc,v 1.8 2012/08/23 02:22:17 manuelf Exp $
//
// Description:
...
TTree *WeightedTree(TString inputfile, double &wentries, TString weightName, double gSR = 0., 
		    TCut cuts="1", TString runs="All", TString newName = "ntp1"){

  WeightManager *myWM;
  if(weightName!="1")  myWM = new WeightManager(weightName,0, false);
  else myWM = new WeightManager(0);
  myWM->gSR = gSR;

  //cout << "Using data from " << inputfile<<", runs "<<runs << " with weights from "<<weightName<<endl;	
  TChain Chain("ntp1");
  if(runs=="All")
    Chain.Add(inputfile);
  else {
	...
	TString fileRun = inputfile; fileRun.ReplaceAll("All",Run);
	Chain.Add(fileRun);
      }
    }
  }
  TTree *c = Chain.CopyTree(cuts);
  if(c==0){
    cout<<"Tree in file "<<inputfile<<" is empty"<<endl;
    wentries = -99.;
    return c;
  }
  bool noFit = true, fitSample = false;
  if(inputfile.Contains("Fit")) noFit = false;
  if(inputfile.Contains("fitSample")) fitSample = true;
  Float_t candM2,candM2NF,candPstarLep,candPLep,mm2pi0,candQ2,candEExtra, candDmass,candBTagDeltam,candBTagDmass,candCosT;
  Float_t candDeltaE, candMES,mpi0,dmpi0,eextrapi0,ppi0,e1pi0, candDeltam, candFitProb, pmisspi0, candPMiss, candPstarD;
  Float_t candThetaLep, candM2Tru, truePLep;
  Int_t MCType,MCSubmode,MCDssmode,MCD,MCPions,MCCombB,MCCombDs,MCDoubleSL, 
    candTruLep,candDstarType,isBzero,isSP6,MCTaumode,trueLepCharge,candType,candIsMu;
  Int_t candTagNeutMult, candTagChargedMult, MCComblongD,MCComblong;
  Float_t truePPi0,trueDssPPi0,CTL,CTV,Chi,Q2,trueDmass, candMvaDl, candMvaComb, candMvaDssDl, candMvaDssComb, candPisoftP=-99.;
  ...
  c->SetBranchAddress("candMvaDssDl",&candMvaDssDl);
  c->SetBranchAddress("candMvaDssComb",&candMvaDssComb);
  ...
  if(noFit)c->SetBranchAddress("mm2pi0",&mm2pi0);
  c->SetBranchAddress("candPstarLep",&candPstarLep);
  c->SetBranchAddress("candType",&candType);

  Float_t weight = -1.;
  long entries = c->GetEntries();
  TTree *treeData = new TTree(newName,"ntp1");
  treeData->Branch("candType",&candType,"candType/I");
  treeData->Branch("MCType",&MCType,"MCType/I");
  treeData->Branch("MCD",&MCD,"MCD/I");
  treeData->Branch("MCPions",&MCPions,"MCPions/I");
  treeData->Branch("MCTaumode",&MCTaumode,"MCTaumode/I");
  treeData->Branch("MCSubmode",&MCSubmode,"MCSubmode/I");
  treeData->Branch("candDstarType",&candDstarType,"candDstarType/I");
  treeData->Branch("candIsMu",&candIsMu,"candIsMu/I");
  treeData->Branch("candTagNeutMult",&candTagNeutMult,"candTagNeutMult/I");
  treeData->Branch("candTagChargedMult",&candTagChargedMult,"candTagChargedMult/I");
  treeData->Branch("candM2",&candM2,"candM2/F");
  treeData->Branch("candM2Tru",&candM2Tru,"candM2Tru/F");
  treeData->Branch("truePLep",&truePLep,"truePLep/F");
  treeData->Branch("candM2NF",&candM2NF,"candM2NF/F");
  treeData->Branch("candQ2",&candQ2,"candQ2/F");
  treeData->Branch("candPMiss",&candPMiss,"candPMiss/F");
  treeData->Branch("candEExtra",&candEExtra,"candEExtra/F");
  treeData->Branch("candPstarLep",&candPstarLep,"candPstarLep/F");
  treeData->Branch("candPLep",&candPLep,"candPLep/F");
  treeData->Branch("candMvaDl",&candMvaDl,"candMvaDl/F");
  treeData->Branch("candThetaLep",&candThetaLep,"candThetaLep/F");
  treeData->Branch("candMvaComb",&candMvaComb,"candMvaComb/F");
  treeData->Branch("candMvaDssDl",&candMvaDssDl,"candMvaDssDl/F");
  treeData->Branch("candMvaDssComb",&candMvaDssComb,"candMvaDssComb/F");
  treeData->Branch("candFitProb",     &candFitProb,    "candFitProb/F");          
  treeData->Branch("candDeltaE",      &candDeltaE,     "candDeltaE/F");          
  treeData->Branch("candDeltam",      &candDeltam,     "candDeltam/F");          
  treeData->Branch("candMES",         &candMES,        "candMES/F");          
  treeData->Branch("candCosT",        &candCosT,       "candCosT/F");          
  treeData->Branch("candBTagDmass",   &candBTagDmass,  "candBTagDmass/F");          
  treeData->Branch("candBTagDeltam",  &candBTagDeltam, "candBTagDeltam/F");          
  treeData->Branch("candDmass",       &candDmass,      "candDmass/F");          
  treeData->Branch("candPstarD",      &candPstarD,     "candPstarD/F");          
  treeData->Branch("candPisoftP",     &candPisoftP,    "candPisoftP/F");          
  treeData->Branch("mpi0",      &mpi0,      "mpi0/F");      
  treeData->Branch("pmisspi0",  &pmisspi0,  "pmisspi0/F");      
  treeData->Branch("dmpi0",     &dmpi0,     "dmpi0/F");      
  treeData->Branch("eextrapi0", &eextrapi0, "eextrapi0/F");      
  treeData->Branch("ppi0",      &ppi0,      "ppi0/F");      
  treeData->Branch("e1pi0",     &e1pi0,     "e1pi0/F");      
  treeData->Branch("weight",    &weight,    "weight/F");
  treeData->Branch("trueCTL",&CTL,"trueCTL/F");
  treeData->Branch("trueCTV",&CTV,"trueCTV/F");
  treeData->Branch("trueChi",&Chi,"trueChi/F");
  treeData->Branch("trueQ2" ,&Q2 ,"trueQ2/F" );
  if(noFit)treeData->Branch("mm2pi0",&mm2pi0,"mm2pi0/F");

  Int_t badW = 0;
  double m2min = -4, m2max = 12, plmin = 0, plmax = 2.4;
  for (int evt = 0 ; evt < entries; evt ++) {
    c->GetEvent(evt);
    if(weightName=="0") weight = 0;
    else if(weightName=="1") weight = 1.;
    else {
      if(candType>4) candType -= 4;
      if (MCType == 0){
	long mask = 1;
	for(long i=0; i<11; i++) mask *= 4;
	int na1 = (MCComblong & (mask + mask*2))/mask;
	int nDs = MCComblongD & 255;
	if(na1==1){
	  if(nDs==1) MCDoubleSL += 10;
	  else if(nDs== 4) MCDoubleSL += 20;
	  else if(nDs==16) MCDoubleSL += 30;
	  else if(nDs==64) MCDoubleSL += 40;
	}
	weight = myWM->getCombWeight(MCCombB,MCCombDs,MCDoubleSL,candType);
      }else
	weight = myWM->getEventWeight(candType,candDstarType,MCType,MCSubmode,MCDssmode,MCD,MCPions,isBzero,isSP6,
				      MCTaumode,candPstarLep,trueDmass,CTL,CTV,Chi,Q2,trueLepCharge,candM2);
      if(MCType>0 && MCType<13){
	if(candMvaDl>BDTCuts[candType-1]) weight *= myWM->getVariWeight("candEExtra", candEExtra, candType);
	else weight *= myWM->getVariWeight("candEExtranoMVA", candEExtra, candType);
      }
//       if(MCType>12 && MCType<15){
// 	if(candMvaDl>BDTCuts[candType-1]) weight /= myWM->getVariWeight("candEExtra", candEExtra, candType);
// 	else weight /= myWM->getVariWeight("candEExtranoMVA", candEExtra, candType);
//       }
      if(MCType>=13&&MCType<=14 && candMvaDssDl>BDTCutspi0[0][candType-1] && candMvaDssComb>BDTCutspi0[1][candType-1]) 
       	weight *= myWM->getBDTBias();
      if(candM2<m2min||candM2>m2max || candPstarLep<plmin||candPstarLep>plmax) continue;
      if(weight<0||weight>100) {badW++; continue;}
      if(weight!=weight) {cout<<"Weight: "<<weight<<", MCType "<<MCType<<", Q2 "<<Q2<<", M2 "<<candM2<<endl;badW++; continue;}
    }
    treeData->Fill();
    wentries += weight;
  }
  if(badW>0.001)cout<<badW<<" entries with weight<0 or weight>100"<<endl;

  //c->Delete();
  delete myWM;
  //cout<< entries<<" entries filled. Weight "<<weight<<endl;
  return treeData;
}

void WeightedTree3(TString inputfile, double &wentries, TString weightName, double gSR = 0., 
		    TCut cuts="1", TString runs="All", TString rootFile = "AWG82/ntuples/temp/temp.root"){

  WeightManager *myWM;
  if(weightName!="1")  myWM = new WeightManager(weightName,0, false);
  else myWM = new WeightManager(0);
  myWM->gSR = gSR;
  TString newName = "ntp1";
  //cout << "Using data from " << inputfile<<", runs "<<runs << " with weights from "<<weightName<<endl;	
  TChain Chain("ntp1");
  if(runs=="All")
    Chain.Add(inputfile);
...
void PlotFinalFit(TString textName, TTree *tree, int nbins = 25, double minx=-0.4, double maxx=1.4,
  ...
  for (int i = 1 ; i <= 58 ; i ++) {
    ...
    TString hname = "keys/root/Fit/pdfKeys_"; hname += i; hname += "_Fit.root";
    TFile hfile(hname); 
...
//     tree->Project(hname,var,cuts); // Element Yields[7][0] changes value!!
    ...
    for(int i=0; i<8; i++) {
      ...
      double scale = Yields[channel+truInd][IndYield[type][i]]*(double)Projbins/
	pdf[Indpdf[channel][i]]->Integral()/(double)nbins;
...
TH1F *GaussConv(TH1F *h, float width, TString hname, int PointsBin, int Cnbins, double CminX, double CmaxX,
  ...
  double val[2000], dx = CwBin/((double)(PointsBin+1)), totAreaG = IntG(0,1,-nSigma,nSigma);
...
void PlotEps(TString hname, TString dataFolder, TString epsFolder, TString weightName="1", TString psEPS = "EPS",
  ...
  Styles style6; 
  style6.setPadsStyle(6); style6.applyStyle();
  ...
  if(!typePlot.Contains("curve")) optionPlot = "same";
  if(doPS){
    TString psname = "keys/eps/"; psname += epsFolder; psname += "/psKeys"; 
  ...
  if(doEPS){
	...
	if(uncert>0){
      ...
      rightLabel[0] += ndof[0]; rightLabel[0] += "; p = "; rightLabel[0] += RoundNumber(Prob(chi2[0],ndof[0])*100,1); rightLabel[0] += "%";    
      rightLabel[1] = "#chi^{2} = "; rightLabel[1] += RoundNumber(chi2[1],1); rightLabel[1] += "/";
      rightLabel[1] += ndof[1]; rightLabel[1] += "; p = "; rightLabel[1] += RoundNumber(Prob(chi2[1],ndof[1])*100,1); rightLabel[1] += "%";    
    }
    TString Mytitle = "Entries/("; Mytitle += RoundNumber(xhigh-xlow,2,(double)nbinx); Mytitle += " GeV^{2})";
    TString Pytitle = "Entries/("; Pytitle += RoundNumber((yhigh-ylow)*1000,0,(double)nbiny); Pytitle += " MeV)";
    ...
    TString epsname = "keys/eps/"; epsname += epsFolder; epsname += "/"; epsname += dataFolder; 
    epsname += "epsKeys"; epsname += Base; epsname += ".eps";
    all6.SaveAs(epsname);
  }
