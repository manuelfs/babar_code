...
//      $Id: DonutAnalysis.cc,v 1.7 2012/08/23 02:12:07 manuelf Exp $
//
// Description:
...
AppResult DonutAnalysis::event( AbsEvent* event )
{
  ...
  int SeedMode, YMode;
  float candIntPur=-1.0,candMES=-99.,candMES_orig=-99.,candMES_ww=-99., candDeltaE=-99.;
  ...
  float candCosT = -99., candM2NF=-99., candPMissNF=-99.;
  ...
  float tempFitChi2, tempM2NF=-99., tempPMissNF=-99.;
...
//   HTValOrderedVector<int> extratracksks;
  ...
  while (anUpsilon = iterUpsilon()) {
    ...
    HepLorentzVector Btagp4 = aBReco->p4(), BtagCM = aBReco->p4();
    Hep3Vector Bp3 = Btagp4.vect(), beamp3 = Upsilon.vect();
    BtagCM.boost(-CMboost);
    Hep3Vector BCMp3 = BtagCM.vect();
    double Ecm = Upsilon.m(), mES_norm=-99., mES_ww=-99.;
    double inSqrt = Ecm*Ecm/4.-BCMp3.mag2();
    if(inSqrt>=0)  mES_norm = sqrt(Ecm*Ecm/4.-BCMp3.mag2());
    else cout<<"Ecm*Ecm/4: "<<cm*Ecm/4<<" and "<<BCMp3.mag2()<<endl;
    double intCalc = Ecm*Ecm/2.+Bp3.dot(beamp3);
    inSqrt = intCalc*intCalc/Upsilon.e()/Upsilon.e()-BtagCM.m2();
    if(inSqrt>=0) mES_ww = sqrt(intCalc*intCalc/Upsilon.e()/Upsilon.e()-Bp3.mag2());
    else cout<<"1st: "<<intCalc*intCalc/Upsilon.e()/Upsilon.e()<<", p_B^2: "<<Btagp4.m2()<<endl;
...
//     if(qvec.mag2()<=4.) continue;
    ...
    if(tempMES>5.27 && candMES<=5.27) keep = true;  // Select first candidates in the Signal Region
    if(candMES>5.27 && tempMES<=5.27) keep = false;
...
//     }
    ...
    wFF = myWMFF->getEventWeight(candType,candDstarType,MCType,MCSubmode,MCDssmode,MCD,MCPions,
			       isBzero,0,MCUnsim,truPPi0,truDssPPi0,trueCTL,trueCTV,trueChi,trueQ2,
			       trueLepCharge,candM2);
    wComb = myWM->getCombWeight(MCCombB,MCCombDs,MCDoubleSL,candType);
    ...
    cands->column("candMES_ww",  candMES_ww);
    ...
    cands->column("candM2NF",candM2NF);
    cands->column("candQ2",candQ2);
    cands->column("candPMiss",candPMiss);
    cands->column("candPMissNF",candPMissNF);
...
// The pipi and KsKs modes are rejected and the D mass has to be within 4sigma of its value
...
// The KKpipi mode is rejected and the D mass has to be within 4sigma of its value
