...
void Final_BDTVar(TString FileTag = "Ne2", bool isError=false, int isIso=0){
...
//       label.SetTextSize(0.072); label.DrawLatex(minRD[pad]-0.6*delta, 1-halfH*(2*file+1)+0.085, fileTag[file]);
	...
	if(file==0 && isIso==0){
	  double delta = 225; if(isError) delta = 220;
	  height = minRD2[pad] + (maxRD2[pad]-minRD2[pad])*(0.91-0.12*(cand>1));
...
//     label.DrawLatex(nomiRD[pad][0], 1.12, "Aver.");
  ...
  TString pName = "public_html/Final_BDTVar"; //pName+=FileTag; 
