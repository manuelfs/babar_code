...
void Fit_CrossV(){
  ...
  //TFile crossFile("babar_code/PlotsThesis/root/hCrossKeys_19_20to180_50to175.root");crossFile.cd();
  TFile crossFile("babar_code/PlotsThesis/root/hCrossKeys_29_20to250_90to200.root");crossFile.cd();
  TH2F *hCross = (TH2F *)(gDirectory->Get("cross"))->Clone("hCross");
  hCross->SetDirectory(0);
  crossFile.Close();
  TH1D *h1Cross[3];
  int binP[] = {1,3,10}, colors[] = {2,4,8};
  ...
  TString legNames[] = {"#rho_{p} = 0.50", "#rho_{p} = 0.92", "#rho_{p} = 1.75"};
  for(int i=0; i<3; i++){
  ...
  treeData->Add("fitSamples/RAll/pdfSample19.root");
  TH2F *h2[3]; double integral[3];
  //TString hNames[] = {"36", "73", "146"};
  ...
  double entries = getWentries(treeData), minX = -1, maxX = 10;
  ...
  TH1F hKeys[3]; TString leg2Names[] = {"#rho_{m} = 0.36", "#rho^{cv}_{m} = 0.73", "#rho_{m} = 1.46"};
