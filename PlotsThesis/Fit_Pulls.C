...
void Fit_Pulls(int doR24=0,double maxPull = 2.8, int plotYields=0, int doRMS1=1){
  int isHalf = 1;
  rmsPull_1 = doRMS1;
  ...
  for(int i=iniFolder; i<nFolders; i++) {
    ...
    int iniRest = (i==2)*7, finRest = maxFiles-(i==1)*7;
  ...
  for(int i=0; i<6; i++) {
	...
	int iniRest = (fold==2)*7, finRest = maxFiles-(fold==1)*7;
	for(int rest=iniRest; rest<finRest; rest++){
  ...
  TString pName = "public_html/Fit_Pulls"; 
  if(doR24==1) pName += "R24";
  else if(doR24==2) pName += "RAll";
  else pName += "R26";
  pName += ".eps";
  c.SaveAs(pName);
