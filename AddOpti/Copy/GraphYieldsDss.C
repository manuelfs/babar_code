...
//      for each Breco mode for the D** sample
//
// Author List:
//      Manuel Franco Sevilla                     Stanford University
//
// Revision History:
//      09/05/23 manuelf -- Adaptation GraphYields.C
//---------------------------------------------------------------------------------

void GraphYieldsDss(TString filename="optimB0truth12346_yieldDssRest6_cuts"){
  ...
  tag.SetTextSize(.04);
  int vari[] = {8, 4, 5, 7, 8};
  int col[] = {4,1,2,3,4,6,28,9,13};
  TString names[] = {"rubbish","D**","D l/#tau #nu","Crossfeed","Combinatoric"};
  TString titles[] = {"B^{0}","B^{+}"};
  TGraph *g[5][2];
  TCanvas c("Graph","Graph",1000,500);
  TLegend leg(.625,.74,.9,.9);
  leg.SetFillColor(0);
  ...
  for(int j=0; j<2; j++) {
	...
	g[i][j]->SetTitle("D** sample: Yields for "+titles[j]);
	g[i][j]->Draw("ALP");
      }else{
	g[i][j]->Draw("LP");   
    ...
    line.DrawLine(30,0,30,1.07*values[j][8][0]);
  }
  tag.SetTextColor(4);
  //tag.DrawText(10,.41,"BSemiExcl, old PID");
  c.SaveAs("babar_code/eps/YieldsDss_"+filename+".eps");
}
