...
#include "EvtGenModels/Evt2HDM.hh"
...
EvtDecayBase* Evt2HDM::clone(){

  return new Evt2HDM;

}

void Evt2HDM::decay( EvtParticle *p ){

  p->initializePhaseSpace(getNDaug(),getDaugs());

  calcamp->CalcAmp(p,_amp2,thdmffmodel);

}

void Evt2HDM::initProbMax() {
...
void Evt2HDM::init(){
  ...
  checkNArg(0,1);
  if ( getNArg()==1 ) thdmffmodel = new Evt2HDMFF(getArg(0));
  else thdmffmodel = new Evt2HDMFF(0);
