...
#include "babar_code/FF/bBToDstaunu.hh"
...
void BToDstaunu::SetNorm(double ml){
  _Normalization = Normalization(ml);
}
...
double BToDstaunu::Gamma_q2Angular(double q2, double ctl, double ctv, double chi, int isDgamma, bool lplus,
  ...
  return sqrt(1-ctl*ctl)*sqrt(1-ctv*ctv)*GF*GF/pow(2*PI,3)*Vcb*Vcb/12./pow(_mB,2)*gamma*pow(q2-ml*ml,2)*pDs/q2;
}
...
// *NOT VALIDATED YET*
double BToDstaunu::ComputetL(double thetaL, int isCLN, double ml) {
  double ctl = cos(thetaL);
...
// *NOT VALIDATED YET*
// This spectrum uses the formula from hep-ph/1203.2654 multiplied by sin(thetaL) to acount for d(costL)
...
void BToDstaunu::SetTanBetamH(double tBmH){
  _gSR = -mb_quark*pow(tBmH,2);
}

void BToDstaunu::SetFF(double rho2, double R1, double R2, double R0, double gSR){
  _rho2 = rho2;
  _R1 = R1; _R2 = R2; _R0 = R0; 
  _gSR   = gSR;
}
