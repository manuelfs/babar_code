...
void mESCorr_Comp(bool isError=false){
  ...
  double RD[nFiles][4][2], RD2[nFiles][4][2];
  double minRD[2] = {99., 99.}, maxRD[2] = {-99., -99.}; 

  TString RDTag[] = {"D^{0}", "D*^{0}", "D^{+}", "D*^{+}"};
  double nomiRD[2][2] = {{0, 0.053}, {0, 0.022}};
  TString fileTag[] = {"030", "050", "075", "100", "120", "150", "200", "250", "300"};

  TString fileName = "babar_code/Systematics/Text/mESCorrection_SRNoMva.txt"; 
...
//   if(minRD[1] < minRD[0]) minRD[0] = minRD[1];
  fileName = "babar_code/Systematics/Text/mESCorrection_SBNoMva.txt"; 
  fstream textFile2; textFile2.open(fileName,fstream::in);
  for(int file=0; file<nFiles; file++){
    TString text = ""; int times = 0;
    while(!text.Contains("sample")) {
      textFile2>>text; 
    ...
    for(int cand=0; cand<4; cand++){
      for(int i=0; i<6; i++) textFile2>>text; 
      textFile2>>text; RD2[file][cand][0] = text.Atof();
      textFile2>>text; textFile2>>text; RD2[file][cand][1] = text.Atof();
      if(RD2[file][cand][0]-RD2[file][cand][1] < minRD[cand%2]) minRD[cand%2] = RD2[file][cand][0]-RD2[file][cand][1];
      if(RD2[file][cand][0]+RD2[file][cand][1] > maxRD[cand%2]) maxRD[cand%2] = RD2[file][cand][0]+RD2[file][cand][1];
      //cout<<fileTag[file]<<"\t"<<RD2[file][cand][0]<<" #pm "<<RD2[file][cand][1]<<endl;
    }
    //cout<<endl;
  }
  ...
  double minRD2[2] = {-0.10, -0.10}, maxRD2[2] = {0.1, 0.1}; 
  if(isError) {
  ...
  for(int pad=0; pad<2; pad++){
	...
	double height = RD[file][cand][0]-RD2[file][cand][0], BDT = fileTag[file].Atof()-4;
	double dH = sqrt(pow(RD[file][cand][1],2)+pow(RD2[file][cand][1],2));
	if(cand>1) BDT += 8;
  ...
  TString pName = "public_html/mESCorr_Comp.eps"; 
  can.SaveAs(pName);
  for(int pad=0; pad<2; pad++){
      hRD[pad]->Delete();
  }
