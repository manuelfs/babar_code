...
void CSampleSingle(int nvari=0, TString type = "dss", TString run = "1234", TString cuts="MvacosT",
  ...
  TCut CombCut[] = {"(MCType==0||MCType>6||MCType == 1 || MCType == 3)","(MCType>=0&&MCType<7||MCType>12)"};
  ...
  TCut D0Cut = "MCType == 1 || MCType == 3";
  TCut Ds0Cut = "MCType == 2 || MCType == 4";
  TCut DpCut = "MCType == 7 || MCType == 9";
  TCut DspCut = "MCType == 8 || MCType == 10";
  TCut AllCutDl[] = {D0Cut,Ds0Cut,DpCut,DspCut};
  double wAllDl[] = {wD0,wDs0,wDp,wDsp};

  D0Cut = "MCType == 1 || MCType == 3 || MCType == 5";
  Ds0Cut = "MCType == 2 || MCType == 4 || MCType == 6";
  DpCut = "MCType == 7 || MCType == 9 || MCType == 11";
  DspCut = "MCType == 8 || MCType == 10 || MCType == 12";
  ...
  TString limits[] = {"(15,-.5,1.5)","(15,1.5,2.5)","(20,-2,12)","(25,0,2)","(15,5.27,5.29)","(15,-0.8,0.8)","(5,0,5)","(12,0,12)","(15,0,1)",
  ...
  TString legTag[] = {"Cont (","Bkg (","D*l#nu (","D#tau#nu (","Cont (","Comb (","Dl#nu (","D** ("};
  int legN[4];
  ...
  for(int i=1; i<2; i++){
	...
	m.Draw(vari[nvari]+">>MCbkg2"+limits[nvari+nlim],totcand+AllCut[ncut]);
	...
	if(ncut==0){
      ...
      for(int ncut=1;ncut<2;ncut++){
	TCut totComb = totcand; totComb+=AllCutDl[ncut+a*2]; totComb*="wFF";
	m.Draw(vari[nvari]+">>MCbkg2"+limits[nvari+nlim],totComb);
	h = (TH1F*)gDirectory->Get("MCbkg2");
	...
	m.Draw(vari[nvari]+">>"+nString+limits[nvari+nlim],totcand+AllCut[ncut]);
    ...
    hs[i].GetXaxis()->SetLabelSize(0.07);
    ...
    label->DrawLatex(0.1,0.93,RatioTitle);
    TLegend *leg;
    if(nvari==8){
      if(type=="dss")
	...
	leg = new TLegend(0.5,.1,0.9,0.3);
      ...
      leg = new TLegend(0.1,.56,0.32,0.9);
    else
    ...
    leg->Draw();
    c.cd();
    TPad *p2 = new TPad("p2","",0,0.02,1,.22);
    p2->Draw();
    p2->cd();
    ...
    c.cd();
    label->SetTextSize(0.057);
    label->DrawLatex(0.68,0.23,xtitles[nvari]);
