...
void TableCollapse(TString tname="", int type=3){
  ...
  for(int k=0; k<2; k++){
    ...
    for(int i=0; i<4; i++){
      ...
      S[1][0+lag] += MYields[row+2][c1];
      S[1][1+lag] += MYields[row+2][c2];
      S[2][0+lag] += MYields[row+4][c1]+MYields[row+3][c1];
      S[2][1+lag] += MYields[row+4][c2]+MYields[row+3][c2];
  ...
  TString tabName = "mycode/DssColapsed/"; tabName+=tag; tabName+=basename;
  fstream Table;
  Table.open(tabName,fstream::out);
  fstream tex;
  tex.open("mycode/DssColapsed/"+tag+"tex"+basename,fstream::out);

  lag = 0;
  TString TaNames[] ={"D pi0", "Oth D**", "D(*) l", "Cross.", "Comb."};
  TString TeNames[] ={"D^{(*)0}\\, \\pi^0", "Other D^{**}", "D^{(*)0} \\,l \\,\\nu", "Cross.", "Comb.",
		      "D^{(*)+}\\, \\pi^0", "Other D^{**}", "D^{(*)+} \\,l \\,\\nu", "Cross.", "Comb."};
  for(int k=0; k<2; k++){
    if(type<3 && k==1) break;
    if(k==1){
