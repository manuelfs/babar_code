...
#include "RooFitModels/RooNDKeysPdf.hh"
#include <fstream>
#include <iostream>
...
int main(int argc, char *argv[]){
  ...
  if (argc < 3 || argc > 7 ) {
    cout << "USAGE: FitPdfSampleKEYS sample smoothing [adaptive=a] [nM2bin=1000] [nPlbin=300] [Times=1]" 
	 << endl;
    return 1;
  }
  TString sam = argv[1];
  TString smooth = argv[2];
  TString adaptive = "a";
  if(argc>3) adaptive = argv[3];
  int nM2bin = 1000;
  if(argc>4)  {TString sM2bin = argv[4]; nM2bin = sM2bin.Atoi();}
  int nPlbin = 300;
  if(argc>5)  {TString sPlbin = argv[5]; nPlbin = sPlbin.Atoi();}
  int Times = 1;
  if(argc>6)  {TString sTimes = argv[6]; Times = sTimes.Atoi();}
...
//   c.SetBranchAddress("trueDmass",&trueDmass);
  ...
  for(int rep = 0; rep < Times; rep++){
    cout<<rep<<" is rep and m2min is "<<m2min<<endl;
