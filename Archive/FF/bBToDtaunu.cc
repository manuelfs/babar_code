...
#include "babar_code/FF/bBToDtaunu.hh"
...
void BToDtaunu::SetNorm(double ml){
...
double BToDtaunu::FromSP8ToThisModel(double q2, double thetaL, double ml) {
  ...
  return cln/isgw2/_Normalization;
}
...
double BToDtaunu::Gamma_q2tL(double q2, double thetaL, double fplus, double fminus, double ml){
  ...
  hs  = 3.*ht*ht;
  hsl = ht*h0;
  double LH = sin(thetaL)*2./3*(q2-ml2)*(0.75*sin(thetaL)*sin(thetaL)*hl + (ml2/(2.*q2)) * 
...
void BToDtaunu::SetTanBetamH(double tBmH){
