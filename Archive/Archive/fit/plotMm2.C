...
void plotmm2(double _peak, double _muL, double _muPeak, double _muR, double _nuL, double _nuR,
		  double _biasOffset1, double _biasSlope1, double _biasCurv1,
		  double _biasOffset2, double _biasSlope2, double _biasCurv2,
		  double _sigLow1, double _sigHigh1, double _sigLow2,
		  double _sigHigh2, double _fracLow, double _fracHigh, TString cname="mmiss")
{                     // peak,  muL, muPeak, muR, nuL, nuR
  //RooDonutSemilep f(    0.6, 0.01,    0.5, 1.2, 0.1,   4,    6.4, -5, 1, 0, 0, 0, 0.01, 1.9, 1, 1, 1, 1);
  //plotmm2(1.7424,5.2532e-01,6.8845e-01,0,2.6395,1.3074,2.6,-2.4377,3e-01,-9.2e-01,3.56e-01,0,2.32,8.17e-01,8.8e-01,4.3e-02,3.1e-02,0)
  ...
  TH1F *h[4];
  TString hnames[] = {"m^{2}_{miss}: 0 < p*_{l} < 1","m^{2}_{miss}: 1 < p*_{l} < 1.4",
		      "m^{2}_{miss}: 1.4 < p*_{l} < 1.8","m^{2}_{miss}: 1.8 < p*_{l} < 2.4"};
  for(int i=0; i<4; i++) {
    TString name ="h"; name+=i;
    h[i] = new TH1F(name,hnames[i],400,-4,4);
  }
  TCanvas c("c","Missing mass",1000,800);
  c.Divide(2,2);
  for(double mm=-4; mm<4; mm+=0.02){
...
void plotCompare(double _peak, double _muL, double _muPeak, double _muR, double _nuL, double _nuR,
		  double _biasOffset1, double _biasSlope1, double _biasCurv1,
		  double _sigLow1, double _sigHigh1, double v) {
  //plotCompare(6.1646e-01,2.9984e-02,4.3997e-01,1.2,1,6.9261,6.3045,-4.98,9.7955e-01,1.9508,0,12)  
  RooDonutSemilep f(_peak, _muL, _muPeak,_muR, _nuL, _nuR,_biasOffset1, _biasSlope1, _biasCurv1,
		  0, 0, 0, _sigLow1,_sigHigh1, 1,1, 1, 1);
  RooDonutSemilep f2(_peak, v, _muPeak, _muR, _nuL, _nuR,_biasOffset1, _biasSlope1, _biasCurv1,
		  0, 0, 0, _sigLow1,_sigHigh1, 1,1, 1, 1);
