...
void Final_RunRD(){
  ...
  for(int pad=0; pad<2; pad++){
    ...
    hRD[pad] = new TH1F(hName, "", 10, minRD[pad] - 0.1*delta, maxRD[pad] + 0.15*delta);
    ...
    if(pad==1){
      ...
      label.SetTextSize(0.07); label.DrawLatex(0.59+x, 0.915+y, "All runs");
  ...
  TString pName = "public_html/Final_RunRD.eps"; 
  can.SaveAs(pName);
  for(int pad=0; pad<2; pad++){
      hRD[pad]->Delete();
  }
