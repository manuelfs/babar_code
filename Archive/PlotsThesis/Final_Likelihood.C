...
void Final_Likelihood(){
  ...
  Styles style; style.setPadsStyle(2); //style.PadLeftMargin
  style.applyStyle();
  ...
  for(int pad=0; pad<2; pad++){
    ...
    hName += "Parab";
    double mu = YieldFactor[pad]*Yield[0][pad+1], sigma = YieldFactor[pad]*Error[pad+1], nSig = 3.55;;
    ...
    hParabola[pad]->SetLineWidth(2); hLikeli[pad]->SetLineWidth(2); 
    hParabola[pad]->SetLineColor(2);
    ...
    style.fixYAxis(hParabola[pad],cPad);
    hLikeli[pad]->Draw("c same");

  }
  double legW = 0.38, legH = 0.17;
  double legX = 1-style.PadRightMargin-0.26, legY = 1-style.PadTopMargin-0.01;
  ...
  TString pName = "public_html/Final_Likelihood.eps"; 
  can.SaveAs(pName);
  for(int pad=0; pad<2; pad++){
