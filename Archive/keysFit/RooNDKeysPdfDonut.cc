 ...
 *    Original File: RooNDKeysPdf.cxx 31258 2009-11-17 22:41:06Z wouter      *
 *    File: $Id: RooNDKeysPdfDonut.cc,v 1.3 2012/03/04 00:33:03 manuelf Exp $
 * Authors:                                                                  *
 ...
 *                                                                           *
 * Revision History:
 *      10/02/11 manuelf -- Adapted to the be included in DonutUtils
 *****************************************************************************/
...
public:
  ...
  RooNDKeysPdfDonut(const char *name, const char *title,
               const RooArgList& varList, RooDataSet& data, 
	       TString options, Double_t rho, Double_t rho2, Double_t nSigma, Bool_t rotate=kTRUE) ;  //manuelf
  ...
  RooNDKeysPdfDonut(const RooNDKeysPdfDonut& other, const char* name=0);
  virtual ~RooNDKeysPdfDonut();

  virtual TObject* clone(const char* newname) const { return new RooNDKeysPdfDonut(*this,newname); }
...
Double_t CrossVali(Bool_t Addf2=kTRUE, Double_t nSigma=3) const; //manuelf
...
protected:
  ...
  Double_t GaussPDF(Double_t x, Double_t sigma) const; //manuelf
  ...
  mutable Double_t _widthFactor2;
...
ClassImp(RooNDKeysPdfDonut)



//_____________________________________________________________________________
RooNDKeysPdfDonut::RooNDKeysPdfDonut(const char *name, const char *title,
			   const RooArgList& varList, RooDataSet& data,
  ...
  _rotate(rotate)
{
  ...
  _widthFactor2 = _widthFactor;

  TIterator* varItr = varList.createIterator() ;
  RooAbsArg* var ;
  for (Int_t i=0; (var = (RooAbsArg*)varItr->Next()); ++i) {
    if (!dynamic_cast<RooAbsReal*>(var)) {
      coutE(InputArguments) << "RooNDKeysPdfDonut::ctor(" << GetName() << ") ERROR: variable " << var->GetName() 
...
RooNDKeysPdfDonut::RooNDKeysPdfDonut(const char *name, const char *title,
			   const RooArgList& varList, RooDataSet& data,
			   TString options, Double_t rho, Double_t rho2, Double_t nSigma, Bool_t rotate) : 
  RooAbsPdf(name,title),
  _varList("varList","List of variables",this),
  _data(data),
  _options(options),
  ...
  _rotate(rotate)
{
  // Same as before, but the first dimension has "rho" as scale factor, and
  // all the other ones have "rho2"
...
{ 
  ...
  if (mirror!=NoMirror) {
    if (mirror!=MirrorBoth) 
      coutW(InputArguments) << "RooNDKeysPdfDonut::RooNDKeysPdfDonut() : Warning : asymmetric mirror(s) no longer supported." << endl;
    _options="m";
  }
...
//_____________________________________________________________________________
RooNDKeysPdfDonut::RooNDKeysPdfDonut(const char *name, const char *title, RooAbsReal& x, RooAbsReal & y,
...
//_____________________________________________________________________________
RooNDKeysPdfDonut::RooNDKeysPdfDonut(const RooNDKeysPdfDonut& other, const char* name) :
...
//_____________________________________________________________________________
RooNDKeysPdfDonut::~RooNDKeysPdfDonut() 
{
...
//_____________________________________________________________________________
RooNDKeysPdfDonut::createPdf(Bool_t firstCall) const
{
...
//_____________________________________________________________________________
RooNDKeysPdfDonut::setOptions() const
{
  ...
  cxcoutD(InputArguments) << "RooNDKeysPdfDonut::setOptions()    options = " << _options 
  ...
  if (_nSigma<2.0) {
    coutW(InputArguments) << "RooNDKeysPdfDonut::setOptions() : Warning : nSigma = " << _nSigma << " < 2.0. "
...
//_____________________________________________________________________________
RooNDKeysPdfDonut::initialize() const
{
  // initialization
  ...
  if(_nDim==0) {
    coutE(InputArguments) << "ERROR:  RooNDKeysPdfDonut::initialize() : The observable list is empty. "
			  << "Unable to begin generating the PDF." << endl;
    assert (_nDim!=0);
  }

  if(_nEvents==0) {
    coutE(InputArguments) << "ERROR:  RooNDKeysPdfDonut::initialize() : The input data set is empty. "
			  << "Unable to begin generating the PDF." << endl;
    assert (_nEvents!=0);
  }
  ...
  _rho.push_back(_widthFactor);     // First dimension scale factor
  _rho.resize(_nDim,_widthFactor2); // All other dimensions use rho2
...
//_____________________________________________________________________________
RooNDKeysPdfDonut::loadDataSet(Bool_t firstCall) const
{
...
//   coutI() << "RooNDKeysPdfDonut::loadDataSet(" << this << ")" 
...
//_____________________________________________________________________________
RooNDKeysPdfDonut::mirrorDataSet() const
{
...
void
//_____________________________________________________________________________
RooNDKeysPdfDonut::loadWeightSet() const
{
  _wMap.clear();
...
//   coutI() << "RooNDKeysPdfDonut::loadWeightSet(" << this << ") : Number of weighted events : " << _wMap.size() << endl;
}


void
//_____________________________________________________________________________
RooNDKeysPdfDonut::calculateShell(BoxInfo* bi) const 
{
...
//   coutI(Contents) << "RooNDKeysPdfDonut::calculateShell() : " 
...
void
//_____________________________________________________________________________
RooNDKeysPdfDonut::calculatePreNorm(BoxInfo* bi) const
{
  //bi->nEventsBMSW=0.;
  //bi->nEventsBW=0.;
  ...
  cxcoutD(Eval) << "RooNDKeysPdfDonut::calculatePreNorm() : " 
...
void
//_____________________________________________________________________________
RooNDKeysPdfDonut::sortDataIndices(BoxInfo* bi) const
{
  ...
  for (Int_t j=0; j<_nDim; j++) { 
    cxcoutD(Eval) << "RooNDKeysPdfDonut::sortDataIndices() : Number of sorted events : " << _sortTVIdcs[j].size() << endl; 
  }
}
...
void
//_____________________________________________________________________________
RooNDKeysPdfDonut::calculateBandWidth() const
{
  cxcoutD(Eval) << "RooNDKeysPdfDonut::calculateBandWidth()" << endl; 

  // non-adaptive bandwidth 
  // (default, and needed to calculate adaptive bandwidth)

  if(!_options.Contains("a")) {
      cxcoutD(Eval) << "RooNDKeysPdfDonut::calculateBandWidth() Using static bandwidth." << endl;
  }    
  ...
  // adaptive width
  if(_options.Contains("a")) {
    cxcoutD(Eval) << "RooNDKeysPdfDonut::calculateBandWidth() Using adaptive bandwidth." << endl;
    ...
    for(Int_t i=0; i<_nEvents; ++i) {
      ...
      vector<Double_t>& x = _dataPts[i];
      Double_t f =  TMath::Power( gauss(x,_weights0)/_nEventsW , -1./(2.*_d) ) ; //Original
      //Double_t f =  TMath::Power( gauss(x,_weights0)/_nEventsW , -1./2.) ; //Abramson
...
Double_t
//_____________________________________________________________________________
RooNDKeysPdfDonut::gauss(vector<Double_t>& x, vector<vector<Double_t> >& weights) const 
{
...
void
//_____________________________________________________________________________
RooNDKeysPdfDonut::loopRange(vector<Double_t>& x, map<Int_t,Bool_t>& ibMap) const
{
...
void
//_____________________________________________________________________________
RooNDKeysPdfDonut::boxInfoInit(BoxInfo* bi, const char* rangeName, Int_t /*code*/) const
{
...
Double_t 
//_____________________________________________________________________________
RooNDKeysPdfDonut::evaluate() const 
{
  _varItr->Reset() ;
  RooAbsReal* var ;
...
Int_t 
//_____________________________________________________________________________
RooNDKeysPdfDonut::getAnalyticalIntegral(RooArgSet& allVars, RooArgSet& analVars, const char* rangeName) const
{
...
Double_t 
//_____________________________________________________________________________
RooNDKeysPdfDonut::analyticalIntegral(Int_t code, const char* rangeName) const
{
  cxcoutD(Eval) << "Calling RooNDKeysPdfDonut::analyticalIntegral(" << GetName() << ") with code " << code 
  ...
  // reset
  if (newBounds) {
    cxcoutD(Eval) << "RooNDKeysPdfDonut::analyticalIntegral() : Found new boundaries ... " << (rangeName?rangeName:"<none>") << endl;
    boxInfoInit(bi,rangeName,code);    
  }
  ...
  if (_mirror && bi->netFluxZ) {
    ...
    cxcoutD(Eval) << "RooNDKeysPdfDonut::analyticalIntegral() : Using mirrored normalization : " << bi->nEventsBW << endl;
    return bi->nEventsBW;
  } 
  ...
  else 
  {
    ...
    cxcoutD(Eval) << "RooNDKeysPdfDonut::analyticalIntegral() : Final normalization : " << norm << " " << bi->nEventsBW << endl;
    return norm;
  }
}


// Returns a measure of the mean integrated squared error of the KEYS estimator, upon
// which the smoothing parameter may be optimized
// If Addf2 is true, then a constant term is added to give a scale to this result
// nsigma is a parameter to make the optimal sigma smaller, since the optimal is typcally
// to smooth. A reasonable value is 3
// Gaussian calculation from Bowman/Azzalini, page 37
Double_t RooNDKeysPdfDonut::CrossVali(Bool_t Addf2, Double_t nSigma) const{
  
  Double_t term1=0, term2=0, term3=0, term4=0, term5=0, term6=0;
  Int_t n = _nEvents;
  vector<Double_t> OptSigma;
  for (Int_t k=0; k<_nDim; k++) OptSigma.push_back(_n * (*_sigmaR)[k]/nSigma);
  //cout<<"sigmax: "<<OptSigma[0]<<" and sigmay: "<<OptSigma[1]<<", _n: "<<_n<<endl;

  if(n<=1) return -999.;
  for(Int_t i=0; i<n; i++){
    vector<Double_t>& point_i  = _dataPts[i];
    vector<Double_t>& weight_i = (*_weights)[i];
    Double_t prod1 = 1.;
    Double_t prod4 = 1.;
    for (Int_t k=0; k<_nDim; k++) {
      prod1 *= GaussPDF(0,sqrt(2.)*weight_i[k]);
      if(Addf2) prod4 *= GaussPDF(0,sqrt(2.)*OptSigma[k]);
    }
    term1 += prod1;
    term4 += prod4;
    for(Int_t j=0; j<n; j++){
      if(i == j) continue;
      vector<Double_t>& point_j  = _dataPts[j];
      vector<Double_t>& weight_j = (*_weights)[j];
      Double_t prod2 = 1., prod3 = 1., prod5 = 1., prod6 = 1.;
      for (Int_t k=0; k<_nDim; k++) {
	prod2 *= GaussPDF(point_i[k]-point_j[k],sqrt(weight_i[k]*weight_i[k]+weight_j[k]*weight_j[k]));
	prod3 *= GaussPDF(point_i[k]-point_j[k],weight_j[k]);
	if(Addf2) prod5 *= GaussPDF(point_i[k]-point_j[k],sqrt(2.)*OptSigma[k]);
	if(Addf2) prod6 *= GaussPDF(point_i[k]-point_j[k],OptSigma[k]);
      }
      term2 += prod2;
      term3 += prod3;     
      term5 += prod5;     
  ...
  //cout<<"term1: "<<term1<<", term2: "<<term2<<", term3: "<<term3<<", term4: "<<term4<<
  //  ", term5: "<<term5<<", term6: "<<term6<<endl;
  Double_t result = term1/n/(n-1)+term2*(n-2)/n/(n-1)/(n-1)-term3*2/n/(n-1);
  if(Addf2) result += term6/n/(n-1);//term4/n/(n-1)+term5*(n-2)/n/(n-1)/(n-1);
  //cout<<"n: "<<n<<", cv: "<<result<<endl;
  return result;
}


Double_t RooNDKeysPdfDonut::GaussPDF(Double_t x, Double_t sigma) const{

  if(sigma <= 0){
    cout<<"Non positive sigma: "<<sigma<<endl;
    return -99.;
  }
  return exp(-x*x/2/sigma/sigma)/_sqrt2pi/sigma;
}
