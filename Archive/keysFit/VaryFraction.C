...
//   int indices[5] = {0,12,20,30,59};
//   TH2F* hSum[5][4][2], *hTemp;
//   TFile f1(hname1); TFile f2(hname2); 
//   for(int i=0; i<5; i++){
//     for(int j=0; j<4; j++){
//       int index = indices[i]+j;
//       if(index==21 || index==23) continue;
//       f1.cd();
//       TString pull = "Pull"; pull += index;
//       TString spull = "sPull"; spull += index;
//       hSum[i][j][0] = (TH2F *)(f1.Get(pull))->Clone(spull);    
//       hSum[i][j][0]->SetDirectory(0);
//       f2.cd();
//       hTemp  = (TH2F *)f2.Get(pull);
//       hSum[i][j][0]->Add(hTemp);
//     }
//   }
//   f1.Close(); f2.Close();
//   TFile fSum("FitAll/SumPulls.root","RECREATE"); fSum.cd();
//   for(int i=0; i<5; i++){
//     for(int j=0; j<4; j++){
//       int index = indices[i]+j;
//       if(index==21 || index==23) continue;
