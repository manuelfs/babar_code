...
void Conclusions_RDx(int nFiles = 5, int AverAll = 0, int Verbose = 0){
  ...
  double LeftMargin = 0.21, padW = (1-LeftMargin)/2., padH = (1-style.PadBottomMargin)/(double)nFiles;
  ...
  TString pName = "public_html/Conclusions_RDx.eps"; 
  can.SaveAs(pName);
  for(int pad=0; pad<2; pad++){
      hRD[pad]->Delete();
  }
