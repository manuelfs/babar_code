...
int main(){
  ...
  TString yTitle[] = {"#epsilon(#bar{B}#rightarrowD^{(*)}#tau^{-}#bar{#nu}_{#tau})/#epsilon_{SM} [%]",
                      "#bar{B}#rightarrowD^{(*)}#tau^{-}#bar{#nu}_{#tau} yield"}, HigName, hName;
  ...
  for(iHig=iniHig; iHig<=finHig; iHig += dHig){
    HigName = "/cms2r0/slac/small/FitRAllHigx"; 
  ...
  for(file=0; file<nFiles; file++){
    ...
    while(!text.Contains("R(D")) {
      textFile>>text; 
      bin++;
      if(bin>1000) {cout<<"R(D) not found in "<<fileName<<endl; return 0;}
    }
  ...
  for(int col=0; col<nCols; col++){
    for(int row=0; row<nRows; row++){
      ...
      double RightMargin = 0.03, TopMargin=0, BottomMargin=bMargin/(bMargin+padH)*row*1.15;
	...
	for(int his=0; his<2; his++) {
      ...
      if(row==1) Histo[col][row][0]->SetXTitle("tan#beta/m_{H^{+}} [GeV^{-1}]");
  ...
  TString pdfName = "plots/Higgs_Yields.pdf";
  can.SaveAs(pdfName);
