...
int main(){
  ...
  TString basefolder = "/cms2r0/slac/";
  TString histoName = basefolder+"keys/eps/CSample/root/Q2_Folded.root";
  TFile *fHistos2=0; if(doRoot) fHistos2 = new TFile(histoName,"RECREATE");

  TString Folder = basefolder+"keys/eps/CSample/root/";
  ...
  TChain MC("ntp1");
  MC.Add(basefolder+"small/FitRAllNewx100_RunAll.root");
  ...
  if(Type==9){ // Reading D**(l/tau)nu histograms
    TString textName = basefolder+"public_html/Uncertainties_BkgDss.txt";
  ...
  double dRows = static_cast<double>(nRows), dCols = static_cast<double>(nCols);
  ...
  for(int col=0; col<nCols; col++){
    ...
    for(int row=0; row<nRows; row++){
      ...
      hTemp = static_cast<TH1F *>(fHistos.Get(hName));
      hName = "Data_"; hName += pad;
      if(hTemp) hData[pad]  = static_cast<TH1F *>(hTemp->Clone(hName));
      else continue;
      hData[pad]->SetDirectory(0);

      // // Adding D+ and D*+ for EExtra
      // if(nPDFs[Type][col]==9 && col==0 && Type==0) {
      //   int candi0 =2+row;
      //   fHistos.cd(); can.cd(0);
      //   hName = "Data"; hName += candi0;
      //   hTemp = static_cast<TH1F *>(fHistos.Get(hName));
      //   hName = "Data_"; hName += pad;
      //   TH1F *hEExtra;
      //   if(hTemp) hEExtra  = static_cast<TH1F *>(hTemp->Clone(hName));
      //   else continue;
      //   hEExtra->SetDirectory(0);
      //   hData[pad]->Add(hEExtra);
      // }
      
      int nBins = hData[pad]->GetNbinsX();
      hName = "hYields";  hName += pad;
      hTemp = static_cast<TH1F *>(fHistos.Get("hYields"));
      if((hTemp && Type<8) || Type==9) hYields[pad]  = static_cast<TH1F *>(hTemp->Clone(hName));
	...
	if(yield<=0){
	  ...
	  hMC[pad][pdf]  = static_cast<TH1F *>(hMC[pad][pdf-1]->Clone(hName));
	} else {
	  hTemp = static_cast<TH1F *>(fHistos.Get(hName));
	  hName = "MC_";  hName += pad; hName += pdf;
	  if(hTemp) hMC[pad][pdf]  = static_cast<TH1F *>(hTemp->Clone(hName));
	  else {cout<<row<<", "<<col<<": PDF "<<pdf<<endl; return 0;}
	...
	if(pdf==5 || pdf==6){
	  ...
	  hCopy[pad][pdf] = static_cast<TH1F *>(hMC[pad][pdf]->Clone(hName)); hCopy[pad][pdf]->SetFillColor(1); 
	...
	for(int isNorm=2; isNorm>=0; isNorm--){
	  ...
	  for(int bin=1; bin<=nBinsE; bin++){
	    ...
	    double dq2 = (hQ2[pad][isNorm]->GetBinLowEdge(bin)-q2)/static_cast<double>(nValues);
	    q2 += dq2/2.;
  ...
  TString pName = "plots/Stability_"; pName += TypeName[Type]; pName += ".pdf"; 
